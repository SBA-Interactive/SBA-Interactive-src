...
<div class="relative group">
	...
	>
		...
		<span class="text-xs uppercase font-bold text-surface-700 dark:text-surface-300">{currentLang}</span>
	</button>
	...
	<div class="absolute right-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
		<div class="bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-lg shadow-xl overflow-hidden min-w-[140px]">
			{#each languages as lang}
				<button 
					class="w-full text-left px-4 py-2 text-sm hover:bg-blue-50 dark:hover:bg-surface-700 transition-colors {currentLang === lang.code ? 'text-blue-600 font-bold' : ''}"
					onclick={() => { currentLang = lang.code; isOpen = false; }}
				>
					{lang.label}
				</button>
			{/each}
